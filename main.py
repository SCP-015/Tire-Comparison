...
#  — Skor F: {f_score:.2f}
    ...
    if not st.session_state['admin_logged_in']:
        ...
        with tab1:
            ...
            with col3:
                st.metric("Total Reviews", len(st.session_state.get('reviews', [])) + 4, "↗️ +2")
            with col4:
                st.metric("Sistem Uptime", "99.9%", "🟢 Normal")
            
            st.markdown("---")
            st.subheader("📈 Grafik Penggunaan")
            st.info("Grafik analytics akan ditampilkan di sini")
        ...
        with tab2:
            ...
            with col1:
                st.markdown("**Tambah Ban Baru**")
                if st.button("➕ Tambah Ban", use_container_width=True):
                    st.info("Form tambah ban akan muncul di sini")
            
            with col2:
                st.markdown("**Kelola Ban Existing**")
                if st.button("✏️ Edit Data Ban", use_container_width=True):
                    st.info("Interface edit ban akan muncul di sini")
            
            st.markdown("---")
            st.subheader("📋 Daftar Ban")
            ban_list = load_ban_list()
            for ban in ban_list:
                with st.expander(f"🛞 {ban['nama']}"):
                    st.json(ban)
        ...
        with tab3:
            ...
            with col1:
                ...
                st.info("Charts penggunaan sistem")
        ...
        with tab4:
            ...
            max_selections = st.slider("Max Ban Selections", 2, 10, 5)
